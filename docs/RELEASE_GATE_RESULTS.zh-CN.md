...
本文记录组合 release gate 的本地运行结果。release gate 覆盖默认 preflight、safety boundary verification、ask evaluation、远程 Git URL analyze smoke、release binary smoke、release manifest verification、真实仓库 benchmark 和真实仓库 evaluation quality gate。
...
- 原始输出位于被 Git 忽略的 `eval/m105-release-gate/`。
