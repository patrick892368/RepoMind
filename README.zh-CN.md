...
RepoMind 当前范围已经通过 v1.0 release gate。门禁覆盖本地仓库分析、GitHub 仓库 URL 分析、中英双语报告、技术栈/API/数据库/调用图提取、`ask`、`trace`、`diagnose`、20 个真实仓库评估样本、安全边界检查、CI 和 release artifact smoke。

Parser 覆盖仍会继续通过真实仓库评估逐步增强。核心目标保持不变：第一次运行在 30 秒内给出可用的仓库理解结果，并且默认不生成业务代码。
...
最新组合 gate 已在 20 个真实仓库上达到最低质量分 `1.0`，代表性仓库 benchmark 均低于 30 秒目标：
...
preflight 和 release gate 的 ask evaluation 默认使用 Go CLI 评估器。`scripts/evaluate-ask.ps1` 保留为兼容 wrapper，并委托到同一个 Go CLI 路径：
...
评估脚本会用英文和中文问题覆盖 API 路由、数据库模型、调用链、处理函数、证据类型和本地证据数量，并生成 `summary.json` 与 `summary.md`。自定义 case 文件见 [Ask Evaluation](docs/ASK_EVALUATION.zh-CN.md)。
