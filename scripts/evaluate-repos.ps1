...
$repos = @(
    ...
    @{ Name = "prisma-examples"; Url = "https://github.com/prisma/prisma-examples.git"; Expected = "Prisma, TypeScript"; ExpectedStack = @("NestJS", "Express", "Next.js", "Vue", "React"); MinRoutes = 1; MinModels = 1; ExpectedRoutes = @("/api/feed", "/api/filterPosts"); ExpectedModels = @("Post", "Account", "Comment", "Location") },
    @{ Name = "symfony-demo"; Url = "https://github.com/symfony/demo.git"; Expected = "Symfony, PHP"; ExpectedStack = @("Symfony"); MinRoutes = 5; MinModels = 0; ExpectedRoutes = @("/blog", "/blog/posts/{slug}", "/blog/comment/{postSlug}/new", "/admin/post", "/profile/edit"); ExpectedModels = @() },
    ...
    @{ Name = "next-saas-starter"; Url = "https://github.com/leerob/next-saas-starter.git"; Expected = "Next.js, React"; ExpectedStack = @("Next.js", "React", "Postgres"); MinRoutes = 4; MinModels = 0; ExpectedRoutes = @("/api/user", "/api/team", "/api/stripe/webhook", "/api/stripe/checkout"); ExpectedModels = @() },
